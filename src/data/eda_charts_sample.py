...
def main():
    ...
    plt.title("Sample Data - Patient Age Distribution (Bin Width = 5 Years)")
    plt.savefig("figs/fig_samples/age-histogram_sample.png")
    plt.show()

    # hue-mapped histogram of age by urbanicity
    plt.figure(figsize=(7, 5))
    sns.histplot(data=df, x="ageinyear", hue="Urbanicity", hue_order = ['R', 'U', 'S', 'W'], element="step", stat="density", common_norm=False, binwidth=5, alpha=0.2)
    plt.title("Sample Data - Patient Age Distribution by Urbanicity (Bin Width = 5 years)")
    ...
    plt.title("Sample Data - Count of Patients by Urbanicity")
    plt.savefig("figs/fig_samples/patient-counts-by-urbanicity_sample.png")
    plt.show()
    ...
    plt.title("Sample Data - Count of Patients by Urbanicity and Age")
    plt.xticks(rotation=0)
    plt.legend(title="Age Groups", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig("figs/fig_samples/patient-counts-by-urbanicity-age_sample.png", bbox_inches='tight')
    plt.show()
